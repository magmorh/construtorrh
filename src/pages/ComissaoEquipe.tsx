...
// Vínculo derivado dos playbook_precos por obra
interface EquipeObra {
  encarregados: ColaboradorInfo[]  // pode ter mais de um enc por obra (diferentes atividades)
  cabos: ColaboradorInfo[]
}
...
// Deduplicar colaboradores por ID
function uniqColabs(colabs: ColaboradorInfo[]): ColaboradorInfo[] {
  const seen = new Set<string>()
  return colabs.filter(c => { if (seen.has(c.id)) return false; seen.add(c.id); return true })
}
...
export default function ComissaoEquipe() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [obrRes, colRes, preRes, proRes, comRes] = await Promise.all([
  ...
  // Para cada obra, coleta todos os encarregado_id e cabo_id distintos dos precos
  const equipePorObra = useMemo((): Map<string, EquipeObra> => {
    ...
    precos.forEach(p => {
      ...
      if (p.encarregado_id) {
        const c = colabsMap.get(p.encarregado_id)
        if (c) eq.encarregados.push(c)
      }
      if (p.cabo_id) {
        const c = colabsMap.get(p.cabo_id)
        if (c) eq.cabos.push(c)
      }
    })
    // Deduplica
    m.forEach((eq, obraId) => {
      eq.encarregados = uniqColabs(eq.encarregados)
      eq.cabos        = uniqColabs(eq.cabos)
    })
    return m
  }, [precos, colabsMap])
  ...
  async function calcularComissoes() {
    ...
    // Para cada obra que tem precos com enc/cabo vinculados
    for (const [obraId, equipe] of equipePorObra.entries()) {
      const temEnc  = equipe.encarregados.length > 0
      const temCabo = equipe.cabos.length > 0
      if (!temEnc && !temCabo) continue

      const prodsObra = producoes.filter(p => p.obra_id === obraId)
      if (prodsObra.length === 0) continue

      // Agrupar produções por playbook_item_id
      const gpi = new Map<string, ProducaoItem[]>()
      prodsObra.forEach(p => {
        if (!p.playbook_item_id) return
        if (!gpi.has(p.playbook_item_id)) gpi.set(p.playbook_item_id, [])
        gpi.get(p.playbook_item_id)!.push(p)
      })

      // Por encarregado: agrupa total de premiação enc
      ...
      for (const [, itens] of gpi.entries()) {
        const item = itens[0]
        // Buscar o preço desta atividade nesta obra
        const po = precos.find(p =>
          p.obra_id === obraId &&
        ...
        // Encarregado vinculado nesta atividade
        if (po.encarregado_id && po.valor_premiacao_enc && po.valor_premiacao_enc > 0) {
          const val = po.valor_premiacao_enc * qEfetiv
          ...
          e.total += val
          e.detalhes.push(`${item.playbook_itens?.descricao}: ${qtdTotal}${item.playbook_itens?.unidade} × R$${po.valor_premiacao_enc.toFixed(2)} = R$${val.toFixed(2)}`)
        }
        // Cabo vinculado nesta atividade
        if (po.cabo_id && po.valor_premiacao_cabo && po.valor_premiacao_cabo > 0) {
          const val = po.valor_premiacao_cabo * qEfetiv
          ...
          e.total += val
          e.detalhes.push(`${item.playbook_itens?.descricao}: ${qtdTotal}${item.playbook_itens?.unidade} × R$${po.valor_premiacao_cabo.toFixed(2)} = R$${val.toFixed(2)}`)
        }
      }

      // Gravar comissão para cada encarregado
      for (const [encId, { total, detalhes }] of totalEncPorPessoa.entries()) {
        ...
        const { error } = await supabase.from('comissoes_equipe_v2').upsert({
          ...
          data_geracao: new Date().toISOString().slice(0,10), observacoes: detalhes.join('\n'),
      ...
      // Gravar comissão para cada cabo
    ...
    else if (gerados === 0) toast.warning('Nenhuma premiação gerada. Verifique se há produção e se os encarregados/cabos estão vinculados nas atividades (Playbooks → Preços por Obra).')
  ...
  async function salvarRetrabalho() {
    ...
    if (error) { toast.error('Erro ao salvar. Execute migration_retrabalho.sql no Supabase.'); console.error(error) }
  ...
  const linhasAtividade = useMemo((): LinhaAtividade[] => {
    ...
    for (const [itemId, itens] of gpi.entries()) {
      const ref = itens[0]
      const po = precos.find(p =>
        p.obra_id === obraCalcSel.id &&
        p.playbook_atividades?.descricao === ref.playbook_itens?.descricao
      )
      ...
      const encNome  = po?.encarregado_id ? (colabsMap.get(po.encarregado_id)?.nome ?? null) : null
      const caboNome = po?.cabo_id        ? (colabsMap.get(po.cabo_id)?.nome ?? null) : null
      linhas.push({
        playbook_item_id: itemId, descricao: ref.playbook_itens?.descricao ?? '—',
        unidade: ref.playbook_itens?.unidade ?? '—', categoria: ref.playbook_itens?.categoria ?? null,
        qtdTotal: itens.reduce((s,p)=>s+p.quantidade,0), itensProducao: itens,
        valorPremioEnc: vEnc, valorPremioCabo: vCabo, totalPremioEnc: tEnc, totalPremioCabo: tCabo,
        encNome, caboNome,
      })
    }
    return linhas.sort((a,b)=>(a.categoria??'Z').localeCompare(b.categoria??'Z')||a.descricao.localeCompare(b.descricao))
  }, [obraCalcSel, producoes, precos, colabsMap])
  ...
  // Equipe da obra selecionada (derivada dos precos)
  const equipeCalc    = obraCalcSel ? (equipePorObra.get(obraCalcSel.id) ?? { encarregados: [], cabos: [] }) : { encarregados: [], cabos: [] }

  // Resumo por pessoa (para o rodapé "quem recebe quanto")
  const resumoEncObra = useMemo(() => {
    if (!obraCalcSel) return new Map<string, number>()
    const m = new Map<string, number>()
    linhasAtividade.forEach(l => {
      if (!l.encNome || l.totalPremioEnc <= 0) return
      const po = precos.find(p =>
        p.obra_id === obraCalcSel.id &&
        p.playbook_atividades?.descricao === l.descricao
      )
      const id = po?.encarregado_id
      if (!id) return
      m.set(id, (m.get(id) ?? 0) + l.totalPremioEnc)
    })
    return m
  }, [linhasAtividade, precos, obraCalcSel])

  const resumoCaboObra = useMemo(() => {
    if (!obraCalcSel) return new Map<string, number>()
    const m = new Map<string, number>()
    linhasAtividade.forEach(l => {
      if (!l.caboNome || l.totalPremioCabo <= 0) return
      const po = precos.find(p =>
        p.obra_id === obraCalcSel.id &&
        p.playbook_atividades?.descricao === l.descricao
      )
      const id = po?.cabo_id
      if (!id) return
      m.set(id, (m.get(id) ?? 0) + l.totalPremioCabo)
    })
    return m
  }, [linhasAtividade, precos, obraCalcSel])
    ...
    <div style={{ padding: '20px 24px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      {aba==='vinculos'&&(
        <div>
          <div style={{ background:'#f0f9ff', border:'1px solid #bae6fd', borderRadius:10, padding:'12px 16px', marginBottom:16, fontSize:13, color:'#0369a1' }}>
            📌 Os vínculos de <strong>Encarregado</strong> e <strong>Cabo</strong> são configurados em <strong>Playbooks → Preços por Obra</strong>, coluna R$ Enc. e R$ Cabo, para cada atividade. Os dados abaixo refletem esses vínculos automaticamente.
          </div>
                  ...
                  <div key={obra.id} style={{ background:'#fff', border:'1px solid #e2e8f0', borderRadius:12, padding:'14px 16px' }}>
                    ...
                    <div style={{ marginBottom:8 }}>
                      ...
                      {eq.encarregados.length===0
                        ?<div style={{ fontSize:12, color:'#94a3b8', fontStyle:'italic' }}>— não vinculado nas atividades —</div>
                        :eq.encarregados.map(c=>(
                          <div key={c.id} style={{ fontSize:13, fontWeight:600, color:'#1e293b', display:'flex', alignItems:'center', gap:6 }}>
                            <HardHat size={13} color="#c2410c"/>{c.nome}
                            {c.chapa&&<span style={{ fontSize:10, color:'#94a3b8', fontFamily:'monospace' }}>({c.chapa})</span>}
                          </div>
                        ))
                      }
                    </div>
                    ...
                    <div>
                      ...
                      {eq.cabos.length===0
                        ?<div style={{ fontSize:12, color:'#94a3b8', fontStyle:'italic' }}>— não vinculado nas atividades —</div>
                        :eq.cabos.map(c=>(
                          <div key={c.id} style={{ fontSize:13, fontWeight:600, color:'#1e293b', display:'flex', alignItems:'center', gap:6, marginBottom:2 }}>
                            <Users size={12} color="#0369a1"/>{c.nome}
                            {c.chapa&&<span style={{ fontSize:10, color:'#94a3b8', fontFamily:'monospace' }}>({c.chapa})</span>}
                          </div>
                        ))
                      }
          ...
          <div style={{ display:'grid', gridTemplateColumns:'280px 1fr', gap:16, alignItems:'start' }}>
            ...
            <div style={{ border:'1px solid #e2e8f0', borderRadius:10, background:'#fff', overflow:'hidden', position:'sticky', top:20 }}>
              <div style={{ padding:'12px 14px', borderBottom:'1px solid #e2e8f0', background:'#f8fafc' }}>
                <p style={{ margin:'0 0 8px', fontSize:13, fontWeight:700, color:'#1e293b', display:'flex', alignItems:'center', gap:6 }}>
                  <Building2 size={13} color="#0d3f56"/> Obras
                </p>
                ...
                {loading?<div style={{ padding:20, textAlign:'center', color:'#94a3b8', fontSize:12 }}>Carregando…</div>
                 :obras.filter(o=>!searchObraCalc||o.nome.toLowerCase().includes(searchObraCalc.toLowerCase())).map(obra=>{
                  ...
                  for(const [,itens] of gpi2.entries()){
                    const ref = itens[0]
                    const po = precos.find(p=>p.obra_id===obra.id&&p.playbook_atividades?.descricao===ref.playbook_itens?.descricao)
                    if(!po) continue
                    itens.forEach(prod=>{ const f=fatorRetrabalho(prod.num_retrabalhos); tRapido+=prod.quantidade*((po.valor_premiacao_enc??0)+(po.valor_premiacao_cabo??0))*f })
                  }
                  return (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow style={{ background:'#f8fafc' }}>
                          ...
                          <TableHead style={{ textAlign:'center', width:120 }}>Enc. Vinculado</TableHead>
                          <TableHead style={{ textAlign:'center', width:120 }}>Cabo Vinculado</TableHead>
                        ...
                        {linhasAtividade.map((linha,idx)=>(
                          <React.Fragment key={linha.playbook_item_id}>
                            ...
                            <TableRow style={{ background:idx%2===0?'transparent':'#fafafa' }}>
                              ...
                              <TableCell>
                                <div style={{ fontWeight:700, fontSize:13, color:'#1e293b' }}>{linha.descricao}</div>
                                <div style={{ fontSize:10, color:'#94a3b8' }}>{linha.itensProducao.length} registro(s)</div>
                              </TableCell>
                              ...
                              <TableCell style={{ textAlign:'center' }}>
                                {linha.encNome
                                  ?<span style={{ fontSize:11, fontWeight:600, color:'#c2410c', background:'#fff7ed', border:'1px solid #fed7aa', borderRadius:20, padding:'2px 8px' }}>👷 {linha.encNome.split(' ')[0]}</span>
                              ...
                              <TableCell style={{ textAlign:'center' }}>
                                {linha.caboNome
                                  ?<span style={{ fontSize:11, fontWeight:600, color:'#0369a1', background:'#f0f9ff', border:'1px solid #bae6fd', borderRadius:20, padding:'2px 8px' }}>🔧 {linha.caboNome.split(' ')[0]}</span>
                                  :<span style={{ fontSize:10, color:'#cbd5e1' }}>—</span>}
                            ...
                            {/* Sub-linhas por registro de produção */}
                    ...
                    <div style={{ display:'flex', gap:24, flexWrap:'wrap', marginBottom:12 }}>
                      ...
                      <div>
                        ...
                        {resumoEncObra.size===0
                          ?<div style={{ fontSize:12, color:'#94a3b8', fontStyle:'italic' }}>— Nenhum encarregado vinculado nas atividades —</div>
                          :[...resumoEncObra.entries()].map(([id, val])=>{
                            const c = colabsMap.get(id)
                            return <div key={id} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                              <HardHat size={13} color="#c2410c"/>
                              <span style={{ fontSize:13, fontWeight:700, color:'#1e293b' }}>{c?.nome??id}</span>
                              <span style={{ fontSize:16, fontWeight:800, color:'#c2410c' }}>{formatCurrency(val)}</span>
                            </div>
                          })
                        }
                      </div>
                      ...
                      <div>
                        ...
                        {resumoCaboObra.size===0
                          ?<div style={{ fontSize:12, color:'#94a3b8', fontStyle:'italic' }}>— Nenhum cabo vinculado nas atividades —</div>
                          :[...resumoCaboObra.entries()].map(([id, val])=>{
                            const c = colabsMap.get(id)
                            return <div key={id} style={{ display:'flex', alignItems:'center', gap:8, marginBottom:3 }}>
                              <Users size={12} color="#0369a1"/>
                              <span style={{ fontSize:13, fontWeight:700, color:'#1e293b' }}>{c?.nome??id}</span>
                              <span style={{ fontSize:15, fontWeight:800, color:'#0369a1' }}>{formatCurrency(val)}</span>
                            </div>
                          })
                        }
                      </div>
                  ...
                  {comissoes.filter(c=>c.obra_id===obraCalcSel.id).length===0?(
                    ...
                    <div style={{ border:'1px solid #e2e8f0', borderRadius:8, overflow:'hidden' }}>
                      <Table>
                        <TableHeader>
                          <TableRow style={{ background:'#f8fafc' }}>
                            <TableHead>Colaborador</TableHead><TableHead style={{ textAlign:'center' }}>Função</TableHead>
                            <TableHead style={{ textAlign:'right', fontWeight:800 }}>💰 Premiação</TableHead>
                            <TableHead style={{ textAlign:'center' }}>Status</TableHead><TableHead style={{ textAlign:'center' }}>Ações</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
        ...
        <DialogContent style={{ maxWidth:420 }}>
          ...
          <div style={{ padding:'12px 0' }}>
            ...
            <div style={{ background:'#fffbeb', border:'1px solid #fde68a', borderRadius:8, padding:'8px 12px', fontSize:12, color:'#92400e' }}>
              <strong>Regra:</strong> 0 → 100% · 1 → 50% · 2+ → perde a premiação desta produção
            </div>
          </div>
          <DialogFooter>
      ...
      <AlertDialog open={!!aprovarCom} onOpenChange={o=>!o&&setAprovarCom(null)}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Aprovar premiação?</AlertDialogTitle>
          <AlertDialogDescription>Prêmio de <strong>{formatCurrency(aprovarCom?.valor_final??0)}</strong> para <strong>{aprovarCom?.colaboradores?.nome}</strong> ({aprovarCom?.funcao}) em {mesLabel(aprovarCom?.competencia??'')}.<br/><br/><details style={{ fontSize:12, color:'#475569' }}><summary style={{ cursor:'pointer', fontWeight:600 }}>Ver detalhes</summary><pre style={{ whiteSpace:'pre-wrap', marginTop:8, fontSize:11 }}>{aprovarCom?.observacoes??'—'}</pre></details></AlertDialogDescription></AlertDialogHeader>
