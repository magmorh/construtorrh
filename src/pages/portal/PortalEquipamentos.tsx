...
export default function PortalEquipamentos() {
  ...
  useEffect(() => {
    ...
    supabase.from('obras').select('id,nome').in('id', obras).order('nome')
      .then(({ data }) => {
        if (data) {
          setObrasData(data)
          if (data.length > 0 && !obraId) setObraId(data[0].id)
        }
      })
  }, [])
  ...
  const fetchRows = useCallback(async () => {
    if (!obraId) return
    setLoading(true)
    const { data, error } = await supabase.from('obra_equipamentos')
      .select('*').eq('obra_id', obraId).order('tipo').order('created_at', { ascending: false })
    if (error) {
  ...
  async function excluir(id: string) {
    ...
    toast.success('Removido'); fetchRows()
  }
  ...
  // Separar por tipo para exibição em seções
  const locados  = rowsFiltrados.filter(r => r.tipo === 'locado')
  const proprios = rowsFiltrados.filter(r => r.tipo === 'proprio')
  ...
  const obraAtual = obrasData.find(o => o.id === obraId)
  ...
  return (
    <PortalLayout>
      ...
      <div style={{ padding: '0 16px 10px' }}>
        ...
        {obrasData.length > 0 ? (
          <select value={obraId} onChange={e => setObraId(e.target.value)}
            style={{ ...I, cursor: 'pointer', border: '2px solid #1e3a5f', borderRadius: 10, fontWeight: 600 }}>
            {obrasData.map(o => <option key={o.id} value={o.id}>{o.nome}</option>)}
          </select>
        ) : (
          <div style={{ height: 40, background: '#f8fafc', border: '1px solid #e5e7eb', borderRadius: 8, display: 'flex', alignItems: 'center', paddingLeft: 12, fontSize: 13, color: '#9ca3af' }}>
            Carregando obras…
          </div>
        )}
      </div>
      ...
      <div style={{ display: 'flex', margin: '0 16px 12px', background: '#f3f4f6', borderRadius: 10, padding: 4 }}>
        {([['lista', '📋 Lista'], ['novo', editId ? '✏️ Editar' : '➕ Novo']] as [string,string][]).map(([k,l]) => (
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              {([
                ...
                ['proprio', '🔧 Ferramenta Própria',  'Comprada para a obra' ],
              ] as [Tipo,string,string][]).map(([t,l,s]) => (
      ...
      {aba === 'lista' && (
        <div style={{ padding: '0 16px 32px' }}>
          ...
          {loading ? (
            ...
            <>
              {/* Seção Locados */}
              {(tipoFiltro==='todos'||tipoFiltro==='locado') && locados.length > 0 && (
                <div style={{marginBottom:16}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8,padding:'6px 0',borderBottom:'2px solid #bfdbfe'}}>
                    <span style={{fontSize:16}}>🚛</span>
                    <span style={{fontWeight:800,fontSize:14,color:'#1d4ed8'}}>Equipamentos Locados</span>
                    <span style={{background:'#dbeafe',color:'#1d4ed8',borderRadius:12,padding:'1px 8px',fontSize:11,fontWeight:700}}>{locados.length}</span>
                  </div>
                  {locados.map(r => <CardEquip key={r.id} r={r} onStatus={marcarStatus} onEdit={abrirEditar} onDelete={excluir}/>)}
                </div>
              )}
              {/* Seção Próprios */}
              {(tipoFiltro==='todos'||tipoFiltro==='proprio') && proprios.length > 0 && (
                <div>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8,padding:'6px 0',borderBottom:'2px solid #bbf7d0'}}>
                    <span style={{fontSize:16}}>🔧</span>
                    <span style={{fontWeight:800,fontSize:14,color:'#15803d'}}>Ferramentas Próprias</span>
                    <span style={{background:'#dcfce7',color:'#15803d',borderRadius:12,padding:'1px 8px',fontSize:11,fontWeight:700}}>{proprios.length}</span>
                  </div>
                  {proprios.map(r => <CardEquip key={r.id} r={r} onStatus={marcarStatus} onEdit={abrirEditar} onDelete={excluir}/>)}
                </div>
              )}
            </>
          )}
...
// ─── Card de item ─────────────────────────────────────────────────────────────
function CardEquip({ r, onStatus, onEdit, onDelete }: {
  r: Equip
  ...
  return (
    <div style={{
      ...
      borderLeft:`4px solid ${vencido?'#dc2626':r.tipo==='locado'?'#2563eb':'#16a34a'}`,
      borderRadius:10, padding:'12px 14px', marginBottom:8,
    }}>
      <div style={{display:'flex',alignItems:'flex-start',gap:10}}>
        <div style={{fontSize:24,flexShrink:0}}>{r.tipo==='locado'?'🚛':'🔧'}</div>
        <div style={{flex:1}}>
          <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap',marginBottom:3}}>
            <span style={{fontWeight:800,fontSize:14,color:'#1e3a5f'}}>{r.nome}</span>
            <span style={{fontSize:10,background:sc.bg,color:sc.cor,borderRadius:5,padding:'1px 7px',fontWeight:700}}>{sc.emoji} {sc.label}</span>
            {vencido && <span style={{fontSize:10,background:'#fee2e2',color:'#dc2626',borderRadius:5,padding:'1px 7px',fontWeight:700}}>⏰ Vencido</span>}
          </div>
          {r.descricao && <div style={{fontSize:11,color:'#6b7280'}}>{r.descricao}</div>}
      ...
      {r.status==='ativo' ? (
        <div style={{display:'flex',gap:6,marginTop:10,flexWrap:'wrap'}}>
          {r.tipo==='locado' && (
            <button onClick={()=>onStatus(r.id,'devolvido')} style={{flex:1,padding:'7px 0',borderRadius:8,border:'1px solid #bfdbfe',background:'#eff6ff',color:'#1d4ed8',fontWeight:700,fontSize:12,cursor:'pointer'}}>↩️ Devolvido</button>
          )}
          {r.tipo==='proprio' && (
            <button onClick={()=>onStatus(r.id,'baixa')} style={{flex:1,padding:'7px 0',borderRadius:8,border:'1px solid #ddd6fe',background:'#f5f3ff',color:'#7c3aed',fontWeight:700,fontSize:12,cursor:'pointer'}}>🗑️ Dar Baixa</button>
          )}
          <button onClick={()=>onStatus(r.id,'defeito')} style={{flex:1,padding:'7px 0',borderRadius:8,border:'1px solid #fecaca',background:'#fef2f2',color:'#dc2626',fontWeight:700,fontSize:12,cursor:'pointer'}}>⚠️ Defeito</button>
          <button onClick={()=>onEdit(r)} style={{padding:'7px 12px',borderRadius:8,border:'1px solid #e5e7eb',background:'#fff',color:'#6b7280',fontWeight:700,fontSize:12,cursor:'pointer'}}>✏️</button>
        </div>
      ) : (
        <div style={{display:'flex',gap:6,marginTop:8}}>
          <button onClick={()=>onStatus(r.id,'ativo')} style={{flex:1,padding:'6px 0',borderRadius:7,border:'1px solid #bbf7d0',background:'#f0fdf4',color:'#15803d',fontWeight:700,fontSize:11,cursor:'pointer'}}>↺ Reativar</button>
          <button onClick={()=>onDelete(r.id)} style={{padding:'6px 12px',borderRadius:7,border:'1px solid #fecaca',background:'#fef2f2',color:'#dc2626',fontWeight:700,fontSize:11,cursor:'pointer'}}><Trash2 size={13}/></button>
        </div>
      )}
    </div>
  )
